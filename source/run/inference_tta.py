...
from source.pipeline.step.tta_retrieval import TTARetrievalStep
...
def build_arg_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path to a baseline *_report_metrics.json. The TTA report "
            "will include quality gains, runtime overhead, and gain-per-day."
        ),
    )
...
if __name__ == '__main__':
    ...
    if opt.running_name is None:
        opt.running_name = (
            f"infer_tta_{opt.dataset}_{opt.tta_level}_{opt.tta_pseudo_label}"
        )
